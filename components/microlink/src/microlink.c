...
skip_bsd_socket:
    ...
    if (ret != pdPASS) {
        const uint32_t derp_stack_fallback = 4 * 1024;
        ESP_LOGW(TAG, "Failed to create derp_tx task (stack=%u), retrying with stack=%u",
                 (unsigned)ML_TASK_DERP_TX_STACK, (unsigned)derp_stack_fallback);
        ret = xTaskCreatePinnedToCore(ml_derp_tx_task, "ml_derp_tx", derp_stack_fallback,
                                      ml, ML_TASK_DERP_TX_PRIO, &ml->derp_tx_task, ML_TASK_DERP_TX_CORE);
    }
    if (ret != pdPASS) {
        const uint32_t derp_stack_min = 3584;
        ESP_LOGW(TAG, "Failed to create pinned derp_tx task, retrying unpinned (stack=%u)",
                 (unsigned)derp_stack_min);
        ret = xTaskCreate(ml_derp_tx_task, "ml_derp_tx", derp_stack_min,
                          ml, ML_TASK_DERP_TX_PRIO, &ml->derp_tx_task);
    }
    if (ret != pdPASS) {
    ...
    if (ret != pdPASS) {
        const uint32_t wg_stack_fallback = 5 * 1024;
        ESP_LOGW(TAG, "Failed to create wg_mgr task (stack=%u), retrying with stack=%u",
                 (unsigned)ML_TASK_WG_MGR_STACK, (unsigned)wg_stack_fallback);
        ret = xTaskCreatePinnedToCore(ml_wg_mgr_task, "ml_wg_mgr", wg_stack_fallback,
                                      ml, ML_TASK_WG_MGR_PRIO, &ml->wg_mgr_task, ML_TASK_WG_MGR_CORE);
    }
    if (ret != pdPASS) {
        const uint32_t wg_stack_min = 4 * 1024;
        ESP_LOGW(TAG, "Failed to create pinned wg_mgr task, retrying unpinned (stack=%u)",
                 (unsigned)wg_stack_min);
        ret = xTaskCreate(ml_wg_mgr_task, "ml_wg_mgr", wg_stack_min,
                          ml, ML_TASK_WG_MGR_PRIO, &ml->wg_mgr_task);
    }
    if (ret != pdPASS) {
