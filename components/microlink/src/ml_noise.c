...
static int noise_aead_encrypt(const uint8_t *key, uint64_t nonce,
    ...
    const uint64_t wireguard_nonce = __builtin_bswap64(nonce);
    chacha20poly1305_encrypt(ciphertext,
                             plaintext ? plaintext : &dummy,
                             pt_len,
                             ad,
                             ad_len,
                             wireguard_nonce,
...
static int noise_aead_decrypt(const uint8_t *key, uint64_t nonce,
    ...
    uint8_t dummy = 0;
    const uint64_t wireguard_nonce = __builtin_bswap64(nonce);
    const bool ok = chacha20poly1305_decrypt(plaintext ? plaintext : &dummy,
                                             ciphertext,
                                             ct_len,
                                             ad,
                                             ad_len,
