 ...
 * Socket timeout is configured outside this callback (connect/data phases).
 * Do not call setsockopt() here: this callback is hot-path and can run under
 * TLS read loops where extra lwIP socket locking risks long stalls/WDT.
...
static void derp_tls_abort_connect(microlink_t *ml, int sock) {
...
esp_err_t ml_derp_connect(microlink_t *ml) {
    ...
    if (ml_connect(sock, res->ai_addr, res->ai_addrlen) < 0) {
        ESP_LOGW(TAG, "TCP connect failed: %d (%s)", errno, strerror(errno));
        ml_close_sock(sock);
        ml_freeaddrinfo(res);
        return ESP_FAIL;
    }
    ...
    if (ret != 0) {
        ...
        ESP_LOGE(TAG, "TLS config defaults failed: -0x%04x (%s)", -ret, err_buf);
        derp_tls_abort_connect(ml, sock);
        return ESP_FAIL;
    }
    ...
    if (ret != 0) {
        char err_buf[128];
        mbedtls_strerror(ret, err_buf, sizeof(err_buf));
        ESP_LOGE(TAG, "TLS setup failed: -0x%04x (%s)", -ret, err_buf);
        derp_tls_abort_connect(ml, sock);
        return ESP_FAIL;
    }
    ...
    if (ret != 0) {
        char err_buf[128];
        mbedtls_strerror(ret, err_buf, sizeof(err_buf));
        ESP_LOGE(TAG, "TLS set hostname failed: -0x%04x (%s)", -ret, err_buf);
        derp_tls_abort_connect(ml, sock);
        return ESP_FAIL;
    }
    ...
    /* Data-phase socket timeouts.
     * Keep socket in blocking mode and rely on SO_RCVTIMEO/SO_SNDTIMEO.
     * This avoids hot busy-loop behavior in mbedTLS read paths when no data
     * is available, while still bounding read/write latency. */
    {
